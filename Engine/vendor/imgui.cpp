...
/*
 ...
 - 2016/05/12 (1.49) - title bar (using ImGuiCol_TitleBg/ImGuiCol_TitleBgActive colors) isn't rendered over a window background (ImGuiCol_WindowBg color) anymore.
                       ...
                       ImVec4 ConvertTitleBgCol(const ImVec4& win_bgcol, const ImVec4& title_bgcol) { float new_a = 1.0f - ((1.0f - win_bgcol.w) * (1.0f - title_bgcol.w)), k = title_bgcol.w / new_a; return ImVec4((win_bgcol.x * win_bgcol.w + title_bgcol.x) * k, (win_bgcol.y * win_bgcol.w + title_bgcol.y) * k, (win_bgcol.z * win_bgcol.w + title_bgcol.z) * k, new_a); }
 ...
 - 2015/07/08 (1.43) - switched rendering data to use indexed rendering. this is saving a fair amount of CPU/GPU and enables us to get anti-aliasing for a marginal cost.
                     ...
                     - the signature of the io.RenderDrawListsFn handler has changed!
                       ...
                       new: ImGui_XXXX_RenderDrawLists(ImDrawData* draw_data).
                         ...
                         ImDrawCmd:  'vtx_count' becomes 'ElemCount', 'clip_rect' becomes 'ClipRect', 'user_callback' becomes 'UserCallback', '_id' becomes 'TextureId'.
 ...
 - 2015/01/11 (1.30) - removed GetDefaultFontData(). uses io.Fonts->GetTextureData*() API to retrieve uncompressed pixels.
                       - old:  const void* pngdata; unsigned int pngsize; ImGui::GetDefaultFontData(NULL, NULL, &pngdata, &pngsize); [..Upload texture to GPU..];
...
void*   ImFileLoadToMemory(const char* filename, const char* mode, size_t* out_file_size, int paddingbytes)
{
    ...
    void* file_data = IM_ALLOC(file_size + paddingbytes);
    if (file_data == NULL)
    {
        ImFileClose(f);
        return NULL;
    ...
    if (paddingbytes > 0)
        memset((void*)(((char*)file_data) + file_size), 0, (size_t)paddingbytes);
...
void ImGui::RenderTextEllipsis(ImDrawList* draw_list, const ImVec2& pos_min, const ImVec2& pos_max, float clip_max_x, float ellipsis_max_x, const char* text, const char* text_end_full, const ImVec2* text_size_if_known)
{
    ...
    if (text_size.x > pos_max.x - pos_min.x)
    {
        ...
        if (ellipsis_char_count > 1)
        {
            ...
            const float spacingbetween_dots = 1.0f * (draw_list->_Data->FontSize / font->FontSize);
            ellipsis_glyph_width = glyph->X1 - glyph->X0 + spacingbetween_dots;
            ellipsis_total_width = ellipsis_glyph_width * (float)ellipsis_char_count - spacingbetween_dots;
        }
...
void ImGui::UpdateMouseMovingWindowNewFrame()
{
    ...
    if (g.MovingWindow != NULL)
    {
        ...
        ImGuiWindow* movingwindow = g.MovingWindow->RootWindow;
...
void ImGui::UpdateHoveredWindowAndCaptureFlags()
{
    ...
    const bool mouse_draggingextern_payload = g.DragDropActive && (g.DragDropSourceFlags & ImGuiDragDropFlags_SourceExtern) != 0;
    if (!mouse_avail_to_imgui && !mouse_draggingextern_payload)
        g.HoveredWindow = g.HoveredRootWindow = NULL;
...
static void FindHoveredWindow()
{
    ...
    ImVec2 paddingregular = g.Style.TouchExtraPadding;
    ImVec2 paddingfor_resize_from_edges = g.IO.ConfigWindowsResizeFromEdges ? ImMax(g.Style.TouchExtraPadding, ImVec2(WINDOWS_RESIZE_FROM_EDGES_HALF_THICKNESS, WINDOWS_RESIZE_FROM_EDGES_HALF_THICKNESS)) : paddingregular;
    for (int i = g.Windows.Size - 1; i >= 0; i--)
    {
        ...
        if (window->Flags & (ImGuiWindowFlags_ChildWindow | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_AlwaysAutoResize))
            ...
            bb.Expand(paddingfor_resize_from_edges);
...
void ImGui::RenderWindowDecorations(ImGuiWindow* window, const ImRect& title_bar_rect, bool title_bar_is_highlight, int resize_grip_count, const ImU32 resize_grip_col[4], float resize_grip_draw_size)
{
    ...
    else
    {
        ...
        {
            ImU32 bgcol = GetColorU32(GetWindowBgColorIdxFromFlags(flags));
            ...
            if (override_alpha)
                bgcol = (bgcol & ~IM_COL32_A_MASK) | (IM_F32_TO_INT8_SAT(alpha) << IM_COL32_A_SHIFT);
            window->DrawList->AddRectFilled(window->Pos + ImVec2(0, window->TitleBarHeight()), window->Pos + window->Size, bgcol, window_rounding, (flags & ImGuiWindowFlags_NoTitleBar) ? ImDrawCornerFlags_All : ImDrawCornerFlags_Bot);
        }
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    const bool window_just_appearingafter_hidden_for_resize = (window->HiddenFramesCannotSkipItems > 0);
    ...
    window->Appearing = (window_just_activated_by_user || window_just_appearingafter_hidden_for_resize);
    if (window->Appearing)
    ...
    if (window_just_appearingafter_hidden_for_resize && !(flags & ImGuiWindowFlags_ChildWindow))
    ...
    if (first_begin_of_the_frame)
    {
        ...
        else if ((flags & ImGuiWindowFlags_Popup) != 0 && !window_pos_set_by_api && window_just_appearingafter_hidden_for_resize)
            window->Pos = FindBestWindowPosForPopup(window);
        ...
        const bool dim_bgfor_modal = (flags & ImGuiWindowFlags_Modal) && window == GetTopMostPopupModal() && window->HiddenFramesCannotSkipItems <= 0;
        const bool dim_bgfor_window_list = g.NavWindowingTargetAnim && (window == g.NavWindowingTargetAnim->RootWindow);
        if (dim_bgfor_modal || dim_bgfor_window_list)
        {
            const ImU32 dim_bgcol = GetColorU32(dim_bgfor_modal ? ImGuiCol_ModalWindowDimBg : ImGuiCol_NavWindowingDimBg, g.DimBgRatio);
            window->DrawList->AddRectFilled(viewport_rect.Min, viewport_rect.Max, dim_bgcol);
        }

        // Draw navigation selection/windowing rectangle background
        if (dim_bgfor_window_list && window == g.NavWindowingTargetAnim)
        {
...
void ImGui::SameLine(float offset_from_start_x, float spacingw)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
    ...
    {
        if (spacingw < 0.0f) spacingw = 0.0f;
        window->DC.CursorPos.x = window->Pos.x - window->Scroll.x + offset_from_start_x + spacingw + window->DC.GroupOffset.x + window->DC.ColumnsOffset.x;
        window->DC.CursorPos.y = window->DC.CursorPosPrevLine.y;
    }
    else
    {
        if (spacingw < 0.0f) spacingw = g.Style.ItemSpacing.x;
        window->DC.CursorPos.x = window->DC.CursorPosPrevLine.x + spacingw;
        window->DC.CursorPos.y = window->DC.CursorPosPrevLine.y;
    }
...
static void ImGui::NavUpdate()
{
    ...
    float nav_scoringrect_offset_y = 0.0f;
    if (nav_keyboard_active)
        nav_scoringrect_offset_y = NavUpdatePageUpPageDown();
    ...
    g.NavScoringRect.TranslateY(nav_scoringrect_offset_y);
...
static float ImGui::NavUpdatePageUpPageDown()
{
    ...
    if (page_up_held != page_down_held || home_pressed != end_pressed) // If either (not both) are pressed
    {
        ...
        else
        {
            ...
            if (IsKeyPressed(g.IO.KeyMap[ImGuiKey_PageUp], true))
            {
                nav_scoringrect_offset_y = -page_offset_y;
            ...
            return nav_scoringrect_offset_y;
        }
    }
    return 0.0f;
}
...
static void ImGui::NavUpdateWindowing()
{
    ...
    bool start_windowingwith_gamepad = !g.NavWindowingTarget && IsNavInputTest(ImGuiNavInput_Menu, ImGuiInputReadMode_Pressed);
    bool start_windowingwith_keyboard = !g.NavWindowingTarget && g.IO.KeyCtrl && IsKeyPressedMap(ImGuiKey_Tab) && (g.IO.ConfigFlags & ImGuiConfigFlags_NavEnableKeyboard);
    if (start_windowingwith_gamepad || start_windowingwith_keyboard)
        if (ImGuiWindow* window = g.NavWindow ? g.NavWindow : FindWindowNavFocusable(g.WindowsFocusOrder.Size - 1, -INT_MAX, -1))
        {
            ...
            g.NavWindowingToggleLayer = start_windowingwith_keyboard ? false : true;
            g.NavInputSource = start_windowingwith_keyboard ? ImGuiInputSource_NavKeyboard : ImGuiInputSource_NavGamepad;
        }
...
bool ImGui::BeginDragDropSource(ImGuiDragDropFlags flags)
{
    ...
    if (!(flags & ImGuiDragDropFlags_SourceExtern))
    {
        ...
        source_dragactive = IsMouseDragging(mouse_button);
    ...
    else
    {
        ...
        source_dragactive = true;
    }

    if (source_dragactive)
    {
        if (!g.DragDropActive)
        {
...
void ImGui::LogRenderedText(const ImVec2* ref_pos, const char* text, const char* text_end)
{
    ...
    const bool lognew_line = ref_pos && (ref_pos->y > g.LogLinePosY + 1);
    if (ref_pos)
...
void ImGui::LogButtons()
{
    ...
    PushID("LogButtons");
#ifndef IMGUI_DISABLE_TTY_FUNCTIONS
    const bool logto_tty = Button("Log To TTY"); SameLine();
#else
    const bool logto_tty = false;
#endif
    const bool logto_file = Button("Log To File"); SameLine();
    const bool logto_clipboard = Button("Log To Clipboard"); SameLine();
...
void ImGui::ShowMetricsWindow(bool* p_open)
{
    ...
    struct Funcs
    {
        ...
        static void NodeDrawCmdShowMeshAndBoundingBox(ImGuiWindow* window, const ImDrawList* draw_list, const ImDrawCmd* draw_cmd, int elem_offset, bool show_mesh, bool show_aabb)
        {
            ...
            ImDrawList* fgdraw_list = GetForegroundDrawList(window); // Render additional visuals into the top-most draw list
            ImDrawIdx* idx_buffer = (draw_list->IdxBuffer.Size > 0) ? draw_list->IdxBuffer.Data : NULL;

            // Draw wire-frame version of all triangles
            ImRect clip_rect = draw_cmd->ClipRect;
            ImRect vtxs_rect(FLT_MAX, FLT_MAX, -FLT_MAX, -FLT_MAX);
            ImDrawListFlags backup_flags = fgdraw_list->Flags;
            fgdraw_list->Flags &= ~ImDrawListFlags_AntiAliasedLines; // Disable AA on triangle outlines is more readable for very large and thin triangles.
            for (unsigned int base_idx = elem_offset; base_idx < (elem_offset + draw_cmd->ElemCount); base_idx += 3)
            {
                ...
                if (show_mesh)
                    fgdraw_list->AddPolyline(triangle, 3, IM_COL32(255, 255, 0, 255), true, 1.0f); // In yellow: mesh triangles
            }
            // Draw bounding boxes
            if (show_aabb)
            {
                fgdraw_list->AddRect(ImFloor(clip_rect.Min), ImFloor(clip_rect.Max), IM_COL32(255, 0, 255, 255)); // In pink: clipping rectangle submitted to GPU
                fgdraw_list->AddRect(ImFloor(vtxs_rect.Min), ImFloor(vtxs_rect.Max), IM_COL32(0, 255, 255, 255)); // In cyan: bounding box of triangles
            }
            fgdraw_list->Flags = backup_flags;
        }
        ...
        static void NodeDrawList(ImGuiWindow* window, ImDrawList* draw_list, const char* label)
        {
            ...
            if (window && IsItemHovered())
                fgdraw_list->AddRect(window->Pos, window->Pos + window->Size, IM_COL32(255, 255, 0, 255));
            if (!node_open)
                return;
            ...
            for (const ImDrawCmd* pcmd = draw_list->CmdBuffer.begin(); pcmd < draw_list->CmdBuffer.end(); elem_offset += pcmd->ElemCount, pcmd++)
            {
                ...
                if (ImGui::IsItemHovered() && (show_drawcmd_mesh || show_drawcmd_aabb) && fgdraw_list)
                ...
                if (ImGui::IsItemHovered() && fgdraw_list)
                ...
                while (clipper.Step())
                    for (int prim = clipper.DisplayStart, idx_i = elem_offset + clipper.DisplayStart * 3; prim < clipper.DisplayEnd; prim++)
                    {
                        ...
                        if (fgdraw_list && ImGui::IsItemHovered())
                        {
                            ImDrawListFlags backup_flags = fgdraw_list->Flags;
                            fgdraw_list->Flags &= ~ImDrawListFlags_AntiAliasedLines; // Disable AA on triangle outlines is more readable for very large and thin triangles.
                            fgdraw_list->AddPolyline(triangle, 3, IM_COL32(255,255,0,255), true, 1.0f);
                            fgdraw_list->Flags = backup_flags;
                        }
                    }
                ImGui::TreePop();
    ...
    if (show_dockingnodes && g.IO.KeyCtrl)
    {
    }
#endif // #ifdef IMGUI_HAS_DOCK
