...
namespace Lava{
	...
	public:
		SpotlightObject() {
			...
			Type = LAVA_ENTITY_TYPE_SPOTLIGHT_OBJECT;
		}
		glm::vec3 GetLightColor(){
			return LightColor;
		}
		glm::vec3 SetLightColor(glm::vec3 lightColor){
		...
		void ApplyPhysicsToEntity() override{
			if (Type == LAVA_ENTITY_TYPE_GAME_OBJECT) {
				if (mRigidBody.IsCreated && mRigidBody.GetBulletRigidBody()->getMass() != 0) {
					...
					worldTransform = mRigidBody.GetBulletRigidBody()->getCenterOfMassTransform();
					...
					Transform.sQuaternion(quaternion);

					if (mRigidBody.GetDebugMesh()->IsCreated) {
						mRigidBody.GetDebugMesh()->Transform.Translate(position);
						mRigidBody.GetDebugMesh()->Transform.sQuaternion(quaternion);
					}
	...
	class CubemapObject : public Entity{
		...
		void CreateDescriptorSets(VkDevice device, VkDescriptorSetLayout descriptorSetLayoutForCubemap,
			...
			for (size_t i = 0; i < DescriptorSets.size(); i++) {
				...
				bufferInfo.offset = 0;
				bufferInfo.range = sizeof(DataTypes::U_Struct_CubemapVP);
		...
		void Init(std::vector<std::string> paths){
			...
			for (size_t i = 0; i < gSwapchain.PGetImageViews()->size(); i++) {
				UniformBuffersMp[i].CreateUniformBuffer(
					gPhysicalDevice.Get(),
					gDevice.Get(),
					sizeof(DataTypes::U_Struct_CubemapVP)
				);
			}
			...
			CreateDescriptorSets(
				...
				*gSetLayoutForCubemapObjects.PGet(),
				gDescriptorPoolForCubemapObjects.Get(),
		...
		void UpdateUniforms(uint32_t imageIndex, VkDevice device) override{
			DataTypes::U_Struct_CubemapVP cubemapMp;
			cubemapMp.view = debugCamera.GetCubemapViewForVulkan();
			cubemapMp.projection = debugCamera.GetProjectionMatrix();
