...
//  Буфер с коммандами для отрисовки примитивов
	...
	static void initVulkan(HWND hwnd, HINSTANCE hInstance){
		...
		gEditor3DView.x = 0;
		gEditor3DView.y = 0;
		gEditor3DView.height = gHeight;
		gEditor3DView.width = gWidth;
		gEditor3DView.minDepth = 0.0f;
		gEditor3DView.maxDepth = 1.0f;
...
//  Данная функция отрисовывает все примитивы на экран
	static void DrawScene(ImDrawData *drawData, Scene &scene)
	{
		...
		//----------------Обновляем uniform'ы--------------------------------------
		for (size_t i = 0; i < scene.gVectorOfEntities().size(); i++) {
			scene.gVectorOfEntities()[i]->UpdateUniforms(imageIndex, gDevice.Get());
		}
		...
		for (size_t i = 0; i < scene.gVectorOfEntities().size(); i++) {
			scene.gVectorOfEntities()[i]->Draw(
	...
	static void clear(){
		...
		gDescriptorPoolForMesh.Destroy(gDevice.Get());
		gDescriptorPoolForRigidBodyMesh.Destroy(gDevice.Get());
		gDescriptorPoolForImgui.Destroy(gDevice.Get());
		gDescriptorPoolForCubemapObjects.Destroy(gDevice.Get());
