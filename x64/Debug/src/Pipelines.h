...
namespace Lava{
	...
	}graphicsPipelineForCubemapObjects;
	...
	class GraphicsPipelineForRigidBodyMesh : public GraphicsPipeline {
		...
		void createGraphicsPipeline(
			VkDevice device,
			VkSwapchainCreateInfoKHR swapchainCreateInfo,
			VkDescriptorSetLayout setLayoutForRigidBodyMesh,
			VkRenderPass renderPass
		) {
			...
			pipelineLayoutInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;
			pipelineLayoutInfo.pSetLayouts = &setLayoutForRigidBodyMesh;
			pipelineLayoutInfo.setLayoutCount = 1;
	...
	}graphicsPipelineForRigidBodyMesh;
}
