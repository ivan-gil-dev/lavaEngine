...
namespace Lava{
	void g_Img_Func_CreateImage(VkPhysicalDevice physicalDevice, VkDevice device, VkImage& image,
		...
		allocateInfo.memoryTypeIndex = VulkanBuffers::gBuf_Func_FindMemoryType(physicalDevice, memRequirements.memoryTypeBits, properties);
	...
	class Texture {
		...
		void CreateTexture(VkPhysicalDevice physicalDevice, VkDevice device, VkQueue commandBufferQueue,
			...
			if (!Pixels) {
				Pixels = stbi_load("assets/decoyTexture.png", &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
			}
			...
			VulkanBuffers::gBuf_Func_CreateBuffer(
				physicalDevice, 
				device, 
				imageSize, 
