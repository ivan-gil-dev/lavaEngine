...
namespace Lava{
	...
	class DebugCamera : public Camera {// Камера для просмотры сцены (Отладочная)
		void MouseUpdate() {
			if (keyPressedEventHandler.IsKeyPressed(LAVA_KEY_M)) {
	...
	public:
		...
		void Update(double deltaTime) {
			if (LAVA_GLOBAL_ENABLE_DYNAMIC_VIEWPORT) {
				ProjectionMatrix = glm::perspective(glm::radians(FOV), ((float)gEditor3DViewport.width / gEditor3DViewport.height), 0.1f, 1000.0f);
			}
