...
namespace Lava{
	class RenderPass{
		...
		void CreateRenderPass(VkDevice device, VkSwapchainCreateInfoKHR swapchainCreateInfo, VkFormat depthImageFormat) {
			...
			colorAttachment.samples = gMSAAsamples;
			colorAttachment.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;
			colorAttachment.finalLayout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;

			VkAttachmentDescription depthAttachment{};
			depthAttachment.format = depthImageFormat;
			depthAttachment.samples = gMSAAsamples;
			...
			if (gMSAAsamples == VK_SAMPLE_COUNT_1_BIT) {
	...
	class GraphicsPipelineForMesh : public GraphicsPipeline {
		...
		void CreateGraphicsPipeline(VkDevice device, VkSwapchainCreateInfoKHR swapchainCreateInfo,
			...
			attribDescription.offset = offsetof(DataTypes::Vertex, pos);
			...
			attribDescription.offset = offsetof(DataTypes::Vertex, texCoord);
			attributeDescriptions.push_back(attribDescription);

			attribDescription.binding = 0;
			attribDescription.location = 3;
			attribDescription.format = VK_FORMAT_R32G32B32_SFLOAT;
			attribDescription.offset = offsetof(DataTypes::Vertex, normals);
			attributeDescriptions.push_back(attribDescription);
			...
			multisampleInfo.rasterizationSamples = gMSAAsamples;
	...
	class GraphicsPipelineForCubemapObjects : public GraphicsPipeline {
			...
			VkDescriptorSetLayout setLayout, VkRenderPass renderPass)
		{
			...
			bindingDescription = {};
			bindingDescription.binding = 0;
			bindingDescription.stride = sizeof(DataTypes::CubemapVertex);
			bindingDescription.inputRate = VK_VERTEX_INPUT_RATE_VERTEX;
			...
			attribDescription.location = 0;
			attribDescription.format = VK_FORMAT_R32G32B32_SFLOAT;
			attribDescription.offset = offsetof(DataTypes::CubemapVertex, pos);
			attributeDescriptions.push_back(attribDescription);
			...
			viewport.x = 0;
			viewport.y = 0;
			viewport.height = gHeight;//(float)swapchainCreateInfo.imageExtent.height;
	...
	}gGraphicsPipelineForCubemapObjects;
	...
	}gGraphicsPipelineForRigidBodyMesh;
}
