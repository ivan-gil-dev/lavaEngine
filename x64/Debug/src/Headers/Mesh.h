...
namespace Lava{
	...
	private:
		...
		std::vector<VulkanBuffers::UniformBuffer>		UniformBuffersPointLightAttributes;
...
public:
		...
		void AddBaseColorTexture(std::string path){
			...
			CreateDescriptorSets(gDevice.Get(), *gSetLayoutForMesh.PGet(), gDescriptorPoolForMesh._Get_(), *gSwapchain.PGetImageViews());
		}
		...
		void CreateMesh(std::string modelPath){
			...
			UniformBuffersPointLightAttributes.resize(gSwapchain.PGetImageViews()->size());
			...
			for (size_t i = 0; i < gSwapchain.PGetImageViews()->size(); i++) {
				...
				UniformBuffersPointLightAttributes[i].CreateUniformBuffer(gPhysicalDevice.Get(), gDevice.Get(), sizeof(DataTypes::U_Struct_SpotlightAttributes));
			...
			CreateDescriptorSets(
				gDevice.Get(), *gSetLayoutForMesh.PGet(), gDescriptorPoolForMesh._Get_(), *gSwapchain.PGetImageViews()
			);
		...
		void UpdateUniforms(uint32_t imageIndex, VkDevice device) {
			if (IsCreated){
				MVP.model = vTransform.GetMatrixProduct();
				...
				vkMapMemory(device, UniformBuffersPointLightAttributes[imageIndex].GetDeviceMemory(), 0, sizeof(DataTypes::U_Struct_SpotlightAttributes), 0, &data);
				memcpy(data, &light, sizeof(DataTypes::U_Struct_SpotlightAttributes));
				vkUnmapMemory(device, UniformBuffersPointLightAttributes[imageIndex].GetDeviceMemory());
		...
		void Destroy() {
			if (IsCreated) {
				mTexture.DestroyTexture(gDevice.Get());
				vkFreeDescriptorSets(gDevice.Get(), gDescriptorPoolForMesh._Get_(), (uint32_t)DescriptorSets.size(), DescriptorSets.data());
				for (size_t i = 0; i < UniformBuffersMVP.size(); i++) {
					UniformBuffersMVP[i].Destroy(gDevice.Get());
					UniformBuffersPointLightAttributes[i].Destroy(gDevice.Get());
	...
	public:
		...
		void CreateMesh(std::string modelPath, glm::vec3 color){
			...
			CreateDescriptorSets(gDevice.Get(), *gSetLayoutForRigidBodyMesh.PGet(), gDescriptorPoolForMesh._Get_(), *gSwapchain.PGetImageViews());
			IsCreated = true;
		}
