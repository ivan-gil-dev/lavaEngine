...
void Engine::Mesh::LoadModel(std::string modelPath) {
    ...
    //Если файл по указанному пути не найден, то создается куб//
    ...
    if (MaterialsFound)
    {
        Shapes.resize(materials.size());
        for (size_t i = 0; i < Shapes.size(); i++)
        {
            Shapes[i].metallicMapPath = reader_config.mtl_search_path + materials[i].reflection_texname;
            Shapes[i].roughnessMapPath = reader_config.mtl_search_path + materials[i].specular_highlight_texname;
            Shapes[i].diffuseMapPath = reader_config.mtl_search_path + materials[i].diffuse_texname;
            Shapes[i].specularMapPath = reader_config.mtl_search_path + materials[i].specular_texname;
            Shapes[i].MatID = (short)i;
        }
    }
    else {
        Shapes.resize(1);
        Shapes[0].metallicMapPath = "";
        Shapes[0].roughnessMapPath = "";

        Shapes[0].diffuseMapPath = "";
        Shapes[0].specularMapPath = "";
        Shapes[0].MatID = 0;
    }

    //Перечисление форм в obj. файле//
    for (size_t s = 0; s < shapes.size(); s++) {
        //Перечисление граней//
        size_t index_offset = 0;
        for (size_t f = 0; f < shapes[s].mesh.num_face_vertices.size(); f++) {
            size_t fv = size_t(shapes[s].mesh.num_face_vertices[f]);
            //Перечисление вершин в грани//
            for (size_t v = 0; v < fv; v++) {
                ...
                //Запись индексов в разные элементы вектора Shapes
                //в зависимости от номера материала
                if (MaterialsFound)
                {
                    Shapes[shapes[s].mesh.material_ids[f]].indexes.push_back(UniqueVertices[vertex]);
                }
                else {
                    Shapes[0].indexes.push_back(UniqueVertices[vertex]);
                }
            }
            index_offset += fv;
    ...
    //Буфер для генерации карты теней
    ...
    //Создание буферов индексов для каждого материала//
    for (size_t i = 0; i < Shapes.size(); i++)
    {
        Shapes[i].indexBuffer.CreateIndexBuffer(renderer.physicalDevice.Get(),
            renderer.device.Get(),
            renderer.device.GetGraphicsQueue(),
            renderer.commandPool.Get(),
            Shapes[i].indexes.data(), sizeof(Shapes[i].indexes[0]) * Shapes[i].indexes.size());
    }
}
...
void Engine::Mesh::CreateDescriptorSets(VkDevice device, VkDescriptorSetLayout descriptorSetLayoutForMesh,
    ...
    for (size_t i = 0; i < DescriptorSets.size(); i++) {
        VkDescriptorBufferInfo bufferInfo{};//Привзяка буфера к дескриптору//
        bufferInfo.buffer = UniformBuffersMVP_b0[i].Get();//дескриптор буфера//
        bufferInfo.offset = 0;
        bufferInfo.range = sizeof(DataTypes::MVP_t);//Размер данных//

        VkWriteDescriptorSet mvpWriteDescriptorSet{};
        mvpWriteDescriptorSet.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
        mvpWriteDescriptorSet.descriptorCount = 1;//Кол-во дескрипторов (зависит от кол-ва буферов)//
        mvpWriteDescriptorSet.descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
        mvpWriteDescriptorSet.dstSet = DescriptorSets[i];//дескриптор набора//
        mvpWriteDescriptorSet.dstBinding = 0;//номер привязки//
        mvpWriteDescriptorSet.dstArrayElement = 0;//позиция в массива (0 - элемент не является массивом)//
        mvpWriteDescriptorSet.pBufferInfo = &bufferInfo;//информация о привязанном буфере//
        ...
        {
            if (j < Shapes.size()) {
                VkDescriptorImageInfo textureInfo{};//информация о текстуре
                textureInfo.imageView = DiffuseTextures_b1[j].GetImageView();//Вид на текстуру
                textureInfo.sampler = DiffuseTextures_b1[j].GetImageSampler();//Сэмплер
                textureInfo.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;//Слой в котором находится изображение
...
void Engine::Mesh::Draw(VkCommandBuffer commandBuffer, int imageIndex, VkPipeline pipeline) {
    if (IsCreated)
    {
        ...
        for (size_t i = 0; i < Shapes.size(); i++)
        {
            ...
            vkCmdBindIndexBuffer(commandBuffer, Shapes[i].indexBuffer.Get(), 0, VK_INDEX_TYPE_UINT32);
            vkCmdDrawIndexed(commandBuffer, (uint32_t)Shapes[i].indexes.size(), 1, 0, 0, 0);
        }
    }
}
            ...
            for (size_t i = 0; i < DiffuseTextures_b1.size(); i++)
            {
                if (i < Shapes.size()) {
                    DiffuseTextures_b1[i].CreateTexture(renderer.physicalDevice.Get(),
                        renderer.device.Get(),
                        renderer.device.GetGraphicsQueue(),
                        renderer.commandPool.Get(), Shapes[i].diffuseMapPath);

                    SpecularTextures_b6[i].CreateTexture(renderer.physicalDevice.Get(),
                        renderer.device.Get(),
                        renderer.device.GetGraphicsQueue(),
                        renderer.commandPool.Get(), Shapes[i].specularMapPath);

                    RoughnessTextures_b9[i].CreateTexture(renderer.physicalDevice.Get(),
                        renderer.device.Get(),
                        renderer.device.GetGraphicsQueue(),
                        renderer.commandPool.Get(), Shapes[i].roughnessMapPath);

                    MetallicTextures_b10[i].CreateTexture(renderer.physicalDevice.Get(),
                        renderer.device.Get(),
                        renderer.device.GetGraphicsQueue(),
                        renderer.commandPool.Get(), Shapes[i].metallicMapPath);
                }
            }
...
void Engine::Mesh::Destroy() {
    if (IsCreated)
    {
        ...
        for (size_t i = 0; i < Shapes.size(); i++) {
            Shapes[i].indexBuffer.Destroy(renderer.device.Get());
        }
