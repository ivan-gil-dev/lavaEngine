...
// Произведение матриц преобразования для источника света
layout(binding = 8) uniform UniformBufferObject2{
    mat4 lightSpaceMatrix;
}ubo2;


layout(location = 0) in vec3 in_Position; // Координаты вершины (Прием)
layout(location = 1) in vec3 in_Color;    // Цвет вершины (Прием)
layout(location = 2) in vec2 in_UVmap;    // Координаты для UV развертки (Прием)
layout(location = 3) in vec3 in_Normals;  // Вектор нормали (Прием)
