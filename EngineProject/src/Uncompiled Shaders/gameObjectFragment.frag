...
vec3 CalculateDirectionalLight(DirectionalLight_t directionalLight_p, vec3 normals_p, vec3 viewDir_p){
    if(directionalLight_p.Color != vec3(0,0,0)){
        vec3 ambient = material.material.ambient * directionalLight_p.Color;
        vec3 lightDir = normalize(-directionalLight_p.Direction);

        float diff = max(dot(normals_p,lightDir), 0.0);
        vec3 diffuse = diff * directionalLight_p.Color * material.material.diffuse;

        vec3 reflectDir = reflect(-lightDir,normals_p);

        float spec = pow(max(dot(viewDir_p,reflectDir),0.0),32);
        vec3 specular = material.material.specular * spec * directionalLight_p.Color;
...
void main(){
    ...
    result *= vec3(texture(baseColorTexture,frag_UVmap));
